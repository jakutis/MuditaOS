...
namespace
{
    ...
    auto getWidestDigitWidth(gui::RawFont *font)
    {
        uint32_t widestWidth = 0;
        for (auto i = 0; i < 10; i++) {
            auto newWidth = font->getPixelWidth(std::to_string(i));
...
namespace gui
{
    ...
    void TimeFixedWidget::setFirst(uint32_t first)
    {
        auto decimal = first / 10;
        if (decimal) {
    ...
    void TimeFixedWidget::setSecond(uint32_t second)
    {
        auto decimal = second / 10;
        if (decimal) {
    ...
    void TimeFixedWidget::setMinus(bool minus)
    {
        minusVisible = minus;
