...
{
  public:
    BedtimeNotification(uint32_t id) : sys::DataMessage{MessageType::MessageTypeUninitialized}, id{id}
    {}

    [[nodiscard]] auto toAction() const -> std::unique_ptr<app::manager::ActionRequest> override
    {
        return std::make_unique<app::manager::ActionRequest>(
            sender, app::manager::actions::ShowPopup, std::make_unique<gui::BedtimeReminderPopupRequestParams>(id));
    }
